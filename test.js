...
window.onload = function () {
    ...
    const grap2 = document.querySelector('#bottom-left-charts')
    const grap3 = document.querySelector('#bottom-right-charts')
    ...
    function loopRev() {
        ...
        // 由于上次loop循环会加1以备下次使用,当轮播图至少需在第一张才能点击-1，若不加判断，连续点击会index负值
        if (index >= 0) index--
    ...
    function axiosOne() {
        ...
        let timerone = setInterval(() => {
            ...
            for (let i = 0; i < seriesArrOneTemp.length; i = i + 2) {
                seriesArrOne.push(seriesArrOneTemp[i])
            }

            for (let i = 0; i < xAxisArrOneTemp.length; i = i + 2) {
